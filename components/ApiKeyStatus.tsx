...
const ApiKeyStatus: React.FC<ApiKeyStatusProps> = ({ activeApiKey, currentUser, assignTokenProcess, onUserUpdate, onOpenChangeServerModal, language }) => {
    ...
    // Add token mode state - Default to hybrid
    ...
    // Sync token mode state with external updates
    useEffect(() => {
    ...
    const toggleTokenMode = () => {
        const newMode = tokenMode === 'hybrid' ? 'personal_only' : 'hybrid';
        setTokenMode(newMode);
                    ...
                    <div className="space-y-3 text-sm">
                        ...
                        <div className="p-2 bg-neutral-100 dark:bg-neutral-800 rounded-md flex justify-between items-center">
                             <div className="flex flex-col">
                                <span className="font-semibold text-neutral-600 dark:text-neutral-300 text-xs">{T.hybridMode}:</span>
                                <span className="text-[10px] text-neutral-500">{T.hybridModeDesc}</span>
                             </div>
                             <label className="relative inline-flex items-center cursor-pointer">
                                <input 
                                    type="checkbox" 
                                    checked={tokenMode === 'hybrid'} 
                                    onChange={toggleTokenMode} 
