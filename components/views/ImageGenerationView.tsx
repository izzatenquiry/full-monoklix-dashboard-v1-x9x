...
import TokenModeToggle from '../common/TokenModeToggle';
...
const ImageGenerationView: React.FC<ImageGenerationViewProps> = ({ onCreateVideo, onReEdit, imageToReEdit, clearReEdit, presetPrompt, clearPresetPrompt, currentUser, onUserUpdate, language }) => {
  ...
  const generateOneImage = async (index: number, attempt = 1, excludedServers: string[] = []) => {
      ...
      const tokenMode = sessionStorage.getItem('monoklix_token_mode') || 'hybrid'; // Default to hybrid
      const isPersonalOnly = tokenMode === 'personal_only';

      // If Personal Mode is active, we let apiClient handle retries (server redundancy), 
      // so we don't pass a specific server URL unless we are in Hybrid mode doing rotations.
      
      // In Hybrid mode, we pick a random server to distribute load and handle our own retries.
      // In Personal mode, we pass undefined to let apiClient use the preferred server + backups.
      ...
      // If Personal Mode and no server selected, serverToUseUrl is undefined, letting apiClient use default+backups.
      ...
      try {
          ...
          if (!isPersonalOnly && attempt < 3 && !isNonRetriable && serverToUseUrl) {
              ...
              // Personal Only Mode OR Max attempts reached OR Non-retriable error: Fail
              // Note: apiClient now handles retries for Personal Mode internally for network errors.
              // So if we get here in Personal Mode, it means even backups failed or it's a 429/400.
