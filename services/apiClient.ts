...
export const executeProxiedRequest = async (
  ...
  // FIX: Default to 'hybrid' mode to ensure reliability for most users
  const tokenMode = sessionStorage.getItem('monoklix_token_mode') || 'hybrid';
  ...
  if (specificToken) {
      ...
      if (!personal) {
          throw new Error("Personal Token Only mode is active, but no personal token is set. Please claim a token in Settings.");
      }
      
      // Phase 1: Try requested server
      addAttempt({ token: personal.token, serverUrl: currentServerUrl, source: 'Personal' });
      
      // Phase 2: Try backup servers (same token) for robustness against server-side 5xx/network errors
      if (!overrideServerUrl) {
          ...
          backupServers.forEach(server => {
              addAttempt({ token: personal.token, serverUrl: server, source: 'Personal' });
          });
      }
  } else {
  ...
  for (let i = 0; i < attempts.length; i++) {
      ...
      try {
          ...
          if (!response.ok) {
              ...
              if (status === 429 && tokenMode === 'personal_only') {
